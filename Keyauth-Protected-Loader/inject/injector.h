...
{
	/////////////////////////////////
	HMODULE nt_dll = LoadLibraryW(xorstr_(L"ntdll.dll"));
	/////////////////////////////////
...
/////////////////////////////////
bool InjectDll(LPCSTR window_class_name, LPCWSTR file_path)
{
	PVOID dll_image = get_dll_by_file(file_path);
	if (!dll_image)
	{
		printf(xorstr_("invalid dll\n"));
		return false;
	}
	PIMAGE_NT_HEADERS dll_nt_head = RtlImageNtHeader(dll_image);
	if (!dll_nt_head)
	{
		printf(xorstr_("pe: invalid pe header\n"));
		return false;
	}
	...
	{
		printf(xorstr_("process not found!\n"));
		return false;
	}
}
/////////////////////////////////
